...
it('does not have any change in priority', async () => {
  await run(
    ...
    'a{ height: 100px; height: 10vh; height: calc(var(--vh, 1vh) * 10); ' + 'color: #FF10GG; }',
    {},
  )
})
...
it('works in another calc', async () => {
  await run(
    ...
    'a{ height: calc(10vh - 100px); ' + 'height: calc(calc(var(--vh, 1vh) * 10) - 100px); }',
    {},
  )
})
...
it('multiple vh in one declaration', async () => {
  await run(
    ...
    'a{ height: calc(10vh * 3 + 20vh); ' + 'height: calc(calc(var(--vh, 1vh) * 10) * 3 + calc(var(--vh, 1vh) * 20));}',
    {},
  )
})
...
it('does not affect !important directive', async () => {
  await run(
    'a{ ' + ' height: 10vh !important; ' + ' max-height: 20vh ! important; ' + ' color: #FF10GG; ' + '}',
    'a{ ' +
